...
import { useDeleteTasksMutation, useEditStatusMutation } from '../features/apiSlice';
...
export default function SingleTask({task}) {
  ...
  const newDate=formatDate(deadline).split(' ')
  ...
  return (
        <div class="lws-task">
            <div class="flex items-center gap-2 text-slate">
              <h2 class="lws-date">{newDate[1]}</h2>
              <h4 class="lws-month">{newDate[0]}</h4>
            </div>
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-2">
                <img src={teamMember.avatar} alt='ferdous' class="team-avater" />
                <p class="lws-task-assignedOn">{teamMember.name}</p>
              </div>

               {status==='completed' && <button class="lws-delete" onClick={handleDelete}>
                  <svg
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke-width="1.5"
               ...
               { status !== 'completed' && <button class="lws-edit" onClick={handleEdit}>
              ...
              <select class="lws-status" value={status} onChange={e=>handleStatus(e.target.value)}>
